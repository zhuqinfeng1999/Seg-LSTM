        ...
        if key_list[0] == 'visual':
            ...
            if key_list[1] == 'class_embedding':
                ...
                new_name = '.'.join(
                    ...
                    #[new_transform_name, 'ln1', key_list[2]])#maybe should be this?
...
def main():
    ...
    if any([s in args.src for s in ['B-16', 'b16', 'base_patch16']]):
        #visual_split = 9 #WHY 9??????????IT'S 12
        visual_split = 12
    elif any([s in args.src for s in ['L-14', 'l14', 'large_patch14']]):
        #visual_split = 18 #WHY 18??????????IT'S 24
        visual_split = 24
    else:
    ...
    weight_test=torch.tensor([1,2,3,4])
    torch.save(weight_test,args.dst)
    print("weight_test has writen to dst")
